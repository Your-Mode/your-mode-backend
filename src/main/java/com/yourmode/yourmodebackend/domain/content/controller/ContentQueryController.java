...
public class ContentQueryController {
    ...
    @Operation(
        summary = "내가 댓글 단 컨텐츠 목록 조회", 
        description = "현재 로그인한 사용자가 댓글을 작성한 컨텐츠 목록을 조회합니다."
    )
    public ResponseEntity<Page<ContentListResponseDto>> getContentsByMyComments(
            @Parameter(
                description = "카테고리 ID 목록 (예: 1,2,3). 여러 카테고리를 선택하면 OR 조건으로 검색됩니다. " +
                             "카테고리 목록: 스타일링 가이드(1), 트렌드 분석(2), 패션 팁(3), 시즌 컬렉션(4), 아이템 추천(5)",
                example = "1,2"
        ...
        return ResponseEntity.ok(contentQueryService.getContentsByUserComments(userDetails.getUserId(), categoryIds, bodyTypeIds, pageable));
    }

    @GetMapping("/my/likes")
    @Operation(
        summary = "내가 좋아요한 컨텐츠 목록 조회", 
        description = "현재 로그인한 사용자가 좋아요를 누른 컨텐츠 목록을 조회합니다."
    )
    public ResponseEntity<Page<ContentListResponseDto>> getContentsByMyLikes(
            @Parameter(
                description = "카테고리 ID 목록 (예: 1,2,3). 여러 카테고리를 선택하면 OR 조건으로 검색됩니다. " +
                             "카테고리 목록: 스타일링 가이드(1), 트렌드 분석(2), 패션 팁(3), 시즌 컬렉션(4), 아이템 추천(5)",
                example = "1,2"
        ...
        return ResponseEntity.ok(contentQueryService.getContentsByUserLikes(userDetails.getUserId(), categoryIds, bodyTypeIds, pageable));
    }
}
